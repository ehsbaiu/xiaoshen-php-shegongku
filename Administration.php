...
$uid=intval($_GET['uid']);
	if($row['uid']!=1){
		msg('此操作只有超级管理员可操作！','Administration.php');
	}
...
$start = ($page - 1) * $pagesize;
	$pages = ceil($DB->count("select count(uid) as count from " . DBQZ . "_user where `administration` = '1'") / $pagesize);
	$sql_user = $DB->query("select * from " . DBQZ . "_user where `administration` = '1' order by uid desc limit $start,$pagesize");
